	...
	return(
      ...
      <div>
        {Numbers.list.map((number,i)=> 
        	<span className={numberClassName(number)} onClick={()=>props.selectNumber(number)} key={i}>{number}</span>)}
      </div>
    </div>
  );
}
...
class Game extends React.Component{
  ...
  const { 
  	...
  	return(
        ...
        <Answer selectedNumbers={selectedNumbers} unselectNumber={this.unselectNumber} /></div>
        <br/>
        {doneStatus ? 
